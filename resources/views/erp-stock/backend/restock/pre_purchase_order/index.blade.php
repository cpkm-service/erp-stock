    ...
    <div class="block-content block-content-full">
        <form name="search">
            <div class="row">
                ...
                <div class="col-12 col-md-6 col-lg-4">
                    <x-backend.select 
                        :children="($fields['pre_purchase_order_statuses_id']['children']??[])" 
                        :options="$fields['pre_purchase_order_statuses_id']['options']" 
                        :text="$fields['pre_purchase_order_statuses_id']['text']" 
                        :name="$fields['pre_purchase_order_statuses_id']['name']" 
                        :placeholder="$fields['pre_purchase_order_statuses_id']['placeholder']"
                        :required="false"
                        :disabled="false"
                        :multiple="false"
                        value="" />
...
<div class="block block-rounded">
    <div class="block-header block-header-default">
        ...
        <a href="{{ route('backend.restock.pre_purchase_order.create') }}" class="btn btn-primary create">{{ __('create') }}</a>
    </div>
    <div class="block-content block-content-full">
...
<script type="text/javascript">
    var url = "{{route('backend.restock.pre_purchase_order.index',[],false)}}";
    var search = makeDataTable(
        ...
        [
            ...
            },
            {
                ...
                title: "{{__('erp-stock::backend.pre_purchase_orders.date')}}"
            },
            {
                data: "no",
                className: "text-md-center",
                title: "{{__('erp-stock::backend.pre_purchase_orders.no')}}"
            },
            {
                data: "name",
                className: "text-md-center",
                title: "{{__('erp-stock::backend.pre_purchase_orders.name')}}"
            },
            {
                data: "staff.name",
                className: "text-md-center",
                title: "{{__('erp-stock::backend.pre_purchase_orders.staff_id')}}"
            },
            {
                data: "department.name",
                className: "text-md-center",
                title: "{{__('erp-stock::backend.pre_purchase_orders.departments_id')}}"
            },
            {
                data: "status.name",
                className: "text-md-center",
                title: "{{__('erp-stock::backend.pre_purchase_orders.pre_purchase_order_statuses_id')}}",
