                ...
                <div class="block-content tab-content">
                    <div class="tab-pane active" id="btabs-static-home" role="tabpanel" aria-labelledby="btabs-alt-static-home-tab">
                        <form action="{{$form['action']}}" method="POST" name="{{$form['name']}}" enctype="multipart/form-data">
                            ...
                            <div class="row">
                                <div class="col-12">
                                    <button class="btn @if($detail->inquiry_order_statuses_id == 1) btn-warning @elseif($detail->inquiry_order_statuses_id == 2) btn-primary @else btn-danger @endif" type="button" id="close" data-id="{{$detail->id}}">{{$detail->status->name}}</button>
                                    @if($detail->inquiry_order_statuses_id == 1 && (!$detail->purchase_order))
                                    <a href="{{route('backend.purchase_orders.create', ['sourceable_type' => \App\Models\InquiryOrder::class, 'sourceable_id' => $detail->id])}}" class="btn btn-success">轉採購單</a>
                                    @else
                                        ...
                                        <table class="table table-striped table-vcenter">
                                            <thead>
                                                <tr>
                                                    ...
                                                    <th class="text-center" style="width:150px;">{{__('backend.inquiry_orders.inquiry_order_items.*.already_count')}}</th>
                                                    <th class="text-center" style="width:150px;">{{__('backend.inquiry_orders.inquiry_order_items.*.yet_count')}}</th>
                                            ...
                                            <tbody id="product_area" data-name="products">
                                                ...
                                                <tr class="product_item template_item ui-sortable-handle">
                                                    ...
                                                    <td class="text-center align-top">
                                                        <input type="checkbox" id="product_test_{{($key +1)}}" disabled @if($item['check']) checked @endif>
                                                    </td>
                                                ...
                                                <table class="table table-bordered w-100">
                                                    ...
                                                    <tbody>
                                                        ...
                                                        <tr>
                                                            ...
                                                            <td class="text-center">{{number_format($item->purchase_order_items->sum('count'))}}</td>
                                                            <th class="text-center">{{number_format($item->count - $item->purchase_order_items->sum('count'))}}</th>
                                                        </tr>
                                                        @endforeach
                                                    </tbody>
                                                </table>
...
<script>
    var url = "{{route('backend.inquiry_orders.index')}}";
    ...
    $(`select[name="sourceable_type"]`).change(function(){
        if($(this).val() == "App\\Models\\SubscriptionOrder") {
            sendApi(`${url}`,"GET",{
            ...
            sendApi(`${url}`,"GET",{
                ...
                result.data.items.map((item, key) => {
                    ...
                    setTimeout(() => {
                        ...
                        $(`input[data-name="items[${i}][count]"]`).attr('disabled', true).val(item.count).trigger('change');
                        $(`input[name="items[${i}][unit]"]`).attr('disabled', true).val(item.unit);
                        $(`input[name="items[${i}][need_date]"]`).attr('disabled', true).val(item.need_date);
                        ...
                        $(`input[name="items[${i}][id]"]`).val(item.id);
                        $(`input[name="items[${i}][file]"]`).attr('disabled', true)
                        $(`#product_stock_${i}`).text(item.stock);
                        if(item.file) {
                    ...
                    // $(`input[name="items[${i}][file]"]`).val('{{asset('storage')}}'+item.file);
                });
            });
        }
    });
    ...
    @else
        @if($detail->inquiry_order_statuses_id == 1)
