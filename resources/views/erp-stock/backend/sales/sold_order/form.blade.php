...
@section('options')
    ...
    <div class="row mb-2">
        <div class="col-12">
            ...
            <a href="{{route('backend.sales.sold_return_order.create', ['sourceable_type' => \Cpkm\ErpStock\Models\Sales\SoldOrder::class, 'sourceable_id' => $detail->id])}}" class="btn btn-success">轉銷貨退回單</a>
            @else
                @if($detail->sold_return_order()->exists())
                <a href="{{route('backend.sales.sold_return_order.show', ['sold_return_order' => $detail->sold_return_order()->first()?->id])}}" class="btn btn-info" target="_blank">銷貨退回單</a>
                @endif
            @endif
        </div>
    </div>
                            ...
                            <table class="table table-bordered w-100">
                                <thead>
                                    <tr>
                                        ...
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.sales_sold_order_items.*.products_id')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.sales_sold_order_items.*.name')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.sales_sold_order_items.*.standard')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.sales_sold_order_items.*.size')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.sales_sold_order_items.*.count')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.sales_sold_order_items.*.unit')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_orders.no')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_return_orders.id')}}</th>
                                        <th class="text-center">{{__('erp-stock::backend.sales_sold_return_orders.sales_sold_return_order_items.*.count')}}</th>
                                ...
                                <tbody>
                                    @php
                                        $already_sales_sold_return_order = 0;
                                        $remainingCount = $item->count;
                                    @endphp
                                    @foreach($item->sales_sold_return_order_items as $key => $sales_sold_return_order_item)                                                            
                                    <tr>
                                        ...
                                        @php
                                            $already_sales_sold_return_order += $sales_sold_return_order_item->count;
                                        @endphp
                                        <th class="text-center">{{ number_format($already_sales_sold_return_order) }}</th>
                                        @php
                                            $remainingCount -= $sales_sold_return_order_item->count;
                                        @endphp        
                                        <td class="text-center">{{ number_format($remainingCount) }}</td>
                                    </tr>
                                    @endforeach
                                </tbody>
                            </table>
