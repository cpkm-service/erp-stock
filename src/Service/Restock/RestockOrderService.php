 ...
 */
class RestockOrderService extends OrderItemService
{
    ...
    /** 
     * @access protected
     * @var RestockOrderRepository
     * @version 1.0
     * @author Henry
    **/
    protected $RestockOrderRepository;
    /** 
     * @access protected
     * @var StaffRepository
     * @version 1.0
    ...
    **/
     ...
     * @author Henry
    **/
    public function __construct(RestockOrder $RestockOrder, Staff $Staff, SystemSetting $SystemSetting, RestockOrderItem $RestockOrderItem) {
        $this->RestockOrderRepository      =   $RestockOrder;
        $this->StaffRepository      =   $Staff;
        $this->SystemSettingRepository = $SystemSetting;
        $this->RestockOrderItemRepository = $RestockOrderItem;
    }
    ...
    public function index($data) {
        ...
        return DataTables::of($this->RestockOrderRepository->listQuery($where))->make();
    }

    public function getRestockOrder($id) {
        return $this->RestockOrderRepository->getDetail($id);
    }
    ...
    public function makeNo($date) {
        ...
        $count = RestockOrder::where('no', 'like', $no."%")->count() + 1;
    ...
    public function store(array $data) {
        return \DB::transaction(function() use ($data){
            ...
            $createData =  Arr::only($data, $this->RestockOrderRepository->getDetailFields());
            $createData['no']   =   $this->makeNo($createData['date']);
            $model     =   $this->RestockOrderRepository->create($createData);
            if(!$model){
    ...
    public function update(array $data, string $id) {
        return \DB::transaction(function() use ($data, $id){
            $data = $this->dataHandle($data);
            $updateData = Arr::only($data, $this->RestockOrderRepository->getDetailFields());
            $model =  $this->getRestockOrder($id);
    ...
    public function dataHandle($data) {
        ...
        if(isset($data['file']) && $data['file'] && $data['file'] instanceof \Illuminate\Http\UploadedFile) {
            $data['file'] = $data['file']->storeAs('restock_orders', date('YmdHis')."-".$data['file']->getClientOriginalName() , 'public');
        }
    ...
    public function setItems($model, $data) {
        ...
        if($data[$key]??false) {
            foreach ($data[$key] as $sort => $item) {
                ...
                if(!$item['purchase_order_items_id']) {
    ...
    public function select($where = []) {
        return $this->RestockOrderRepository->select(['id', 'name', 'no'])->where($where)->get()->map(function($item) {
            return [
                'value' =>  $item->id,
