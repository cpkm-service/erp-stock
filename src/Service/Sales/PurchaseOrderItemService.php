...
use App\Models\SalesPurchaseOrderItem;
...
class PurchaseOrderItemService
{
    /** 
     * @access protected
     * @var SalesPurchaseOrderItemRepository
     * @version 1.0
     * @author Henry
    **/
    protected $SalesPurchaseOrderItemRepository;
     ...
     * @author Henry
    **/
    public function __construct(SalesPurchaseOrderItem $SalesPurchaseOrderItem) {
        $this->SalesPurchaseOrderItemRepository      =   $SalesPurchaseOrderItem;
    }
    ...
    public function index($data) {
        ...
        return DataTables::of($this->SalesPurchaseOrderItemRepository->listQuery($where))->make();
    }

    public function getSalesPurchaseOrderItem($id) {
        return $this->SalesPurchaseOrderItemRepository->getDetail($id);
    }
    ...
    public function makeNo($date) {
        ...
        $count = SalesPurchaseOrderItem::where('no', 'like', $no."%")->count() + 1;
    ...
    public function store(array $data) {
        return \DB::transaction(function() use ($data){
            ...
            $createData =  Arr::only($data, $this->SalesPurchaseOrderItemRepository->getDetailFields());
    ...
    public function setItems($model, $data) {
        ...
        if($data[$key]??false) {
            foreach ($data[$key] as $sort => $item) {
                ...
                $sales_quote_order_item = $this->SalesPurchaseOrderItemRepository->find($item['id']);
                if(!isset($item['file'])) {
    ...
    public function update(array $data, string $id) {
        return \DB::transaction(function() use ($data, $id){
            $data = $this->dataHandle($data);
            $updateData = Arr::only($data, $this->SalesPurchaseOrderItemRepository->getDetailFields());
            $model =  $this->getSalesPurchaseOrderItem($id);
