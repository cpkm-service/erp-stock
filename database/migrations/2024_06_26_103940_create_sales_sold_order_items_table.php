...
return new class extends Migration
{
    ...
    {
        Schema::dropIfExists('sales_sold_order_items');
        Schema::create('sales_sold_order_items', function (Blueprint $table) {
            $table->id();
            $table->morphs('sourceable', 'sourceable');
            $table->unsignedTinyInteger('type')->nullable()->comment('產品類型');
            ...
            $table->unsignedBigInteger('sales_order_items_id')->nullable()->comment('銷貨單項目編號');
            $table->timestamps();
            $table->comment('銷貨憑單項目');
        });
    }
